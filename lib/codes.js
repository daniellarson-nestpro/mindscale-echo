...
export const SEND_FAILED_MESSAGE =
  'We couldn’t send your code just now. Give it a minute and try again.';

/**
 * What POST /api/auth/start answers once the mail provider has been asked.
 * In production an unsent code must not come back as "check your email":
 * the customer would wait for a message that never arrives. Outside
 * production the code is logged to the console, so the flow still works.
 */
export function startResponseAfterSend({ email, sent, production }) {
  if (sent || !production) return { status: 200, body: startOkBody(email) };
  return { status: 503, body: { ok: false, error: 'send_failed', message: SEND_FAILED_MESSAGE } };
}
...
export function verifySuccessBody({ furthestStep, redirectTo }) {
  return {
    ok: true,
    verified: true,
    furthestStep,
