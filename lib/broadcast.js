...
export function startBroadcast(canvas, wrap) {
  ...
  const frame = (t) => {
    ...
    nodes.forEach((n) => {
      ...
      const settle = t > 4.9 ? 1 - clamp01((t - 4.9) / 0.7) : 1;
      intensity *= settle;

      // Never fully inert: a slow ambient shimmer keeps the lattice alive
      // between pulses, so landing mid-settle still reads as a live system.
      const ambient = 0.045 * (0.5 + 0.5 * Math.sin(t * 1.7 + n.tw));
      const alpha = 0.115 + ambient + intensity * 0.82;
    ...
    hubs.forEach((h, i) => {
      ...
      const fade = t > 4.9 ? 1 - clamp01((t - 4.9) / 0.8) : 1;
      ...
      if (h.label && p >= 1 && !compact) {
        ...
        const hold = t > 4.9 ? Math.max(0, 1 - (t - 4.9) / 0.8) : 1;
    ...
    if (!compact && t > 3.4) {
      ...
      const fade = t > 5.0 ? Math.max(0, 1 - (t - 5.0) / 0.8) : 1;
    ...
    // ---- origin pin ----
    const ignite =
      t < 0.35 ? Math.sin((t / 0.35) * Math.PI) : t > 5.9 ? (t - 5.9) / 0.3 : 0;
  ...
  /**
   * Reduced-motion users get the composed still plus an explicit control —
   * motion on demand rather than motion withheld. Plays exactly one cycle.
   */
  let replayBtn = null;
  const addReplayControl = () => {
    replayBtn = document.createElement('button');
    replayBtn.type = 'button';
    replayBtn.textContent = 'Play the broadcast';
    replayBtn.setAttribute(
      ...
      [
        ...
        'color:rgba(127,240,192,0.95)',
        'background:rgba(127,240,192,0.09)',
        'box-shadow:inset 0 0 0 1px rgba(127,240,192,0.28)',
      ].join(';')
    );
