...
export function articleSourceFromLead(lead) {
  if (lead?.article_source) return lead.article_source;
  if (lead?.article_url) return 'url';
  if (lead?.article_text) return 'paste';
  return 'paste';
}

export function buildComposePayload({ lead, articleText, orderId = '', composeRunId = '' } = {}) {
  const payload = {
    ...
    // V1: URL scrape is not an active path; send empty string
    articleUrl: '',
