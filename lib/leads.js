 ...
 *
 * The two WHERE clauses do different jobs and must not be conflated: the first
 * is mutual exclusion (is a run happening right now?), the second is necessity
 * (do we need a new draft at all?). compose_finished_at IS NULL in the second
 * means the last attempt produced nothing, so a retry is allowed.
 *
 * compose_json is deliberately NOT cleared here. n8n can time out, and wiping
 * the customer's existing draft before we know whether the replacement will
 * arrive loses paid work. The in-flight guard is compose_finished_at = NULL,
 * which canReuseN8nCompose already honours; the old draft stays readable on
 * /preview and /account until saveComposeSuccess replaces it.
 */
  ...
  const rows = await sql`
    ...
    WHERE id = ${leadId}
      ...
      AND (
        ...
        OR compose_finished_at IS NULL
        OR updated_at > compose_finished_at
      )
    RETURNING *
  `;
...
/**
 * Release the compose lock after a run that produced no draft.
 *
 * Deliberately touches neither compose_finished_at nor updated_at. That field
 * pair is how briefNewerThanCompose decides whether the brief has moved on
 * since the last real draft; stamping either one here would claim a draft was
 * produced when none was, and canReuseN8nCompose would then serve the stale
 * draft as if it were current. Clearing compose_started_at is the whole job.
 */
export async function releaseComposeLock(leadId) {
  const sql = await getSql();
  if (!sql || !leadId) return null;
  const rows = await sql`
    UPDATE leads
    SET compose_started_at = NULL
    WHERE id = ${leadId}
    RETURNING *
  `;
  return rows[0] || null;
...
export async function upsertLead(email, rawFields = {}) {
  ...
  const rows = await sql`
    ...
    ON CONFLICT (email) DO UPDATE SET
      ...
      article_source = COALESCE(EXCLUDED.article_source, leads.article_source),
...
/**
 * Point the lead at its newest compose run. POST /api/checkout reads
 * `current_compose_run_id` to stamp `orders.compose_run_id`, which is what ties
 * a paid order back to the exact payload that produced its draft.
 */
export async function setCurrentComposeRun(leadId, composeRunId) {
  const sql = await getSql();
  if (!sql || !leadId || !composeRunId) return null;
  try {
    const rows = await sql`
      UPDATE leads SET
        current_compose_run_id = ${composeRunId},
        updated_at = now()
      WHERE id = ${leadId}
      RETURNING id, current_compose_run_id
    `;
    return rows[0] || null;
  } catch (err) {
    console.error('[leads] setCurrentComposeRun failed:', err?.message);
    return null;
  }
}
