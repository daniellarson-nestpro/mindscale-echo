 ...
 *
 * A PDF source carries its extracted text on `.text` (POST /api/article/upload
 * puts it there). `.value` is only ever the filename and must never be used as
 * article text. A PDF whose text could not be extracted -- a scan, or outlined
 * vector type -- yields source `pdf` with empty text, and compose then fails
 * honestly rather than writing a release from a filename.
 */
export function articlePatchFromSources(sources = []) {
  const list = Array.isArray(sources) ? sources : [];
  const url = list.find((s) => s && s.type === 'url');
  const textSource = list.find((s) => s && s.type === 'text');
  const pdf = list.find((s) => s && s.type === 'file');

  const patch = { articleUrl: String(url?.value || '').trim(), articleText: '', articleSource: '' };

  const pasted = String(textSource?.value || '').trim();
  if (pasted) {
    patch.articleText = pasted;
    patch.articleSource = pdf ? 'pdf' : 'paste';
    return patch;
  }
  if (pdf) {
    patch.articleText = String(pdf.text || '');
    patch.articleSource = 'pdf';
  }
  return patch;
}

/** Compose needs real words. A filename is not enough; an extracted PDF is. */
export function hasUsableArticleSource(sources = []) {
  const list = Array.isArray(sources) ? sources : [];
  const pasted = String(list.find((s) => s && s.type === 'text')?.value || '').trim();
  const extracted = String(list.find((s) => s && s.type === 'file')?.text || '').trim();
  return Math.max(pasted.length, extracted.length) >= MIN_ARTICLE_CHARS;
}
