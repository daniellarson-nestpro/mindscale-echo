...
/**
 * One in-flight n8n run per lead. Returns claimed:false if a draft already
 * exists or another compose is still running (caller should wait / reuse).
 */
export async function claimComposeLock(leadId) {
  ...
  const rows = await sql`
    ...
    SET compose_started_at = now(),
        ...
        updated_at = now()
    WHERE id = ${leadId}
      AND (compose_json IS NULL OR compose_json = '')
      AND (
