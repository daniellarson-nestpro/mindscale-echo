...
export function verificationCodeCopy({ code, codeDisplay, loginUrl }) {
  const digits = String(code || '').replace(/\D/g, '');
  const pretty = codeDisplay || digits;
  const subject = `Your Mindscale Echo code: ${pretty}`;
  const text = [
  ...
  const html = `
        ...
        <p style="font-size:28px;letter-spacing:0.12em;font-weight:700">${escapeHtml(pretty)}</p>
...
export async function sendVerificationCodeEmail({ to, loginUrl, code, codeDisplay }) {
  ...
  const copy = verificationCodeCopy({ code, codeDisplay, loginUrl });
