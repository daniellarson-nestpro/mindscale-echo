...
import { syncLeadOntoPaidOrder } from './leads';
...
export async function upsertOrderFromCheckoutSession(session) {
  ...
  const composeRunIdFromMeta = session?.metadata?.compose_run_id || null;
  ...
  const rows = await sql`
    ...
    ON CONFLICT (stripe_session_id) DO UPDATE SET
      ...
      lead_id = COALESCE(orders.lead_id, EXCLUDED.lead_id),
      compose_run_id = COALESCE(orders.compose_run_id, EXCLUDED.compose_run_id),
      order_status = CASE WHEN EXCLUDED.payment_status = 'paid' THEN 'paid' ELSE orders.order_status END,
      updated_at = now()
    RETURNING *
  `;
  ...
  try {
    return await syncLeadOntoPaidOrder(order);
  } catch (err) {
    console.error('[orders] lead attach failed:', err?.message);
    return order;
  }
}
