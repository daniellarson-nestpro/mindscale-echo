...
// Magic-byte signatures for non-SVG types
const SIGNATURES = [
...
function checkMimeSignature(buffer, declaredMime) {
  if (declaredMime === 'image/svg+xml') return true; // text; no reliable magic bytes
  ...
  return true; // unknown mime — skip signature check
}
