 ...
 * The exact request payload is saved BEFORE the model call.
...
/**
 * Get a compose_run by ID. Validates ownership by leadId when one is given.
 *
 * The two cases are written out as two whole queries on purpose. Neon's tagged
 * template does not compose fragments: a nested sql`...` interpolated into
 * another template is sent as a bind parameter, which yields
 * `WHERE id = $1 $2 LIMIT 1` and a syntax error on every call.
 */
