...
export async function notifyOwnerApproval({ leadId, email, orderId }) {
  const paid = Boolean(orderId);
  const action = paid
    ? 'Action required: manually submit to vendor.'
    : 'Draft approved before payment. Wait for checkout, then submit to vendor.';
  const text =
    `*Customer Approved* ✅\n` +
    `Lead: ${leadId}\n` +
    `Order: ${orderId || '—'}\n` +
  ...
  const plain = paid
    ? `Customer ${email} approved their press release. Lead: ${leadId}. Order: ${orderId}. Ready for manual vendor submission.`
    : `Customer ${email} approved their press release before payment. Lead: ${leadId}. Waiting for checkout before vendor submission.`;
  const [tg, em] = await Promise.allSettled([sendTelegram(text), sendOwnerEmail(subject, plain)]);
